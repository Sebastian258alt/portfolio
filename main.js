...
(function () {
  ...
  // localStorage pode lançar erro (modo privado, cookies bloqueados) — nunca deixar partir o site
  ...
  // Valores dinâmicos usados nos textos como {years} e {certs}
  const TOKENS = {
    get years() { return SITE_FACTS.teachingYears; },
    get certs() { return CERTIFICATES_DATA.length; },
  };
  function fmt(str) {
  ...
  const state = {
    lang: safeGet(STORAGE_LANG) || "pt",
    theme: safeGet(STORAGE_THEME) || getPreferredTheme(),
    certFilter: "all",
    certQuery: "",
  };
  ...
  function t(dict) {
    ...
    if (dict == null) return "";
    if (typeof dict === "string") return fmt(dict);
    return fmt(dict[state.lang] || dict.pt || dict.en || "");
  }
  ...
  // <img> com fallback: se o ficheiro falhar, mostra o placeholder em vez de imagem partida
  function safeImg(src, alt, fallback) {
    const img = el("img", { src: src || fallback, alt: alt, loading: "lazy" });
  ...
  function applyLanguage() {
    ...
    document.querySelectorAll("[data-i18n]").forEach((node) => {
      ...
      if (value != null) node.textContent = fmt(value);
    });
  ...
  function renderCertificates() {
    ...
    }).sort((a, b) => (b.issued || "").localeCompare(a.issued || "")); // mais recente primeiro
    ...
    const PLACEHOLDER = "assets/certs/placeholder-cert.svg";
    filtered.forEach((cert) => {
      const catLabel = CERTIFICATE_CATEGORIES.find((cc) => cc.id === cert.category);
      const actions = [];
      if (cert.image) {
        actions.push(el("a", { href: cert.image, target: "_blank", rel: "noopener", class: "btn btn-ghost btn-sm" }, [c.view]));
      }
      if (cert.extra && cert.extra.image) {
        actions.push(el("a", { href: cert.extra.image, target: "_blank", rel: "noopener", class: "btn btn-ghost btn-sm" }, [t(cert.extra.label)]));
      }
      if (cert.pdf) {
        actions.push(el("a", { href: cert.pdf, target: "_blank", rel: "noopener", class: "btn btn-primary btn-sm" }, [c.download]));
      }
      const meta = cert.date ? `${cert.institution} · ${cert.date}` : cert.institution;
      container.appendChild(
        el("article", { class: "card cert-card" }, [
          el("div", { class: "cert-thumb" }, [safeImg(cert.image, `${cert.name} — ${cert.institution}`, PLACEHOLDER)]),
          el("div", { class: "cert-body" }, [
            el("span", { class: "cert-cat" }, [t(catLabel)]),
            el("h3", null, [cert.name]),
            el("span", { class: "cert-meta" }, [meta]),
            el("p", { class: "cert-desc" }, [t(cert.description)]),
            actions.length ? el("div", { class: "cert-actions" }, actions) : null,
          ]),
        ])
      );
    });
  ...
  // devolve o ficheiro do idioma activo (ou o único ficheiro, se for string)
  function docFile(doc, lang) {
    if (!doc.file) return null;
    if (typeof doc.file === "string") return doc.file;
    return doc.file[lang] || doc.file.pt || doc.file.en || null;
  }
  ...
  function renderDocuments() {
    ...
    const otherLang = state.lang === "pt" ? "en" : "pt";
    container.innerHTML = "";
    DOCUMENTS_DATA.forEach((doc) => {
      const isPrivate = doc.visibility === "private";
      const file = docFile(doc, state.lang);
      const altFile = docFile(doc, otherLang);
      const actions = [];
      if (isPrivate || !file) {
        actions.push(el("a", { href: "#contact", class: "btn btn-ghost btn-sm" }, [c.view]));
      } else {
        actions.push(el("a", { href: file, target: "_blank", rel: "noopener", class: "btn btn-ghost btn-sm" }, [c.view]));
        actions.push(el("a", { href: file, download: "", class: "btn btn-primary btn-sm" }, [c.download]));
      }
      const info = [
      ...
      if (!isPrivate && altFile && altFile !== file) {
        info.push(el("a", { href: altFile, target: "_blank", rel: "noopener", class: "doc-alt" }, [c.otherLang]));
      }
      container.appendChild(
        el("div", { class: "card doc-card" }, [
          ...
          el("div", { class: "doc-info" }, info),
        ])
      );
    });
  }

  // Botão "Descarregar CV" do hero: liga ao PDF do idioma activo
  function renderHeroCv() {
    const btn = document.getElementById("hero-cv-btn");
    if (!btn) return;
    const cv = DOCUMENTS_DATA.find((d) => d.id === "doc-cv");
    const file = cv && docFile(cv, state.lang);
    if (file) {
      btn.setAttribute("href", file);
      btn.setAttribute("download", "");
    } else {
      btn.setAttribute("href", "#documents");
  ...
  /* ---------------------------------------------------------------
     RENDER: MOMENTS (fotografias de eventos + lightbox)
  --------------------------------------------------------------- */
  function openLightbox(src, alt) {
    ...
    document.getElementById("lightbox-cap").textContent = alt;
  ...
  function renderMoments() {
    ...
    if (!container || typeof MOMENTS_DATA === "undefined") return;
    const c = SITE_CONTENT[state.lang].moments;
    container.innerHTML = "";
    [...MOMENTS_DATA]
      .sort((a, b) => (b.issued || "").localeCompare(a.issued || "")) // sem data → no fim
      .forEach((m) => {
        const photos = m.photos.map((p) => {
          const alt = t(p.alt);
          const btn = el("button", { type: "button", class: "moment-photo", "aria-label": `${c.enlarge}: ${alt}` }, [
            el("img", { src: p.src, alt: alt, loading: "lazy", width: "1600", height: "1066" }),
          ]);
          btn.addEventListener("click", () => openLightbox(p.src, alt));
          return btn;
        });
        const links = [];
        if (m.source) {
          links.push(
            el("span", { class: "moment-source" }, [
              `${c.source} `,
              el("a", { href: m.source.url, target: "_blank", rel: "noopener" }, [m.source.label]),
            ])
          );
        }
        if (m.related) {
          links.push(el("a", { href: "#certificates", class: "btn btn-ghost btn-sm" }, [c.certLink]));
        }
        const meta = [m.date && t(m.date), m.place && t(m.place)].filter(Boolean).join(" · ");
        container.appendChild(
          el("article", { class: "moment" + (photos.length === 1 ? " moment--single" : "") }, [
            el("div", { class: "moment-photos", "data-count": String(photos.length) }, photos),
            el("div", { class: "moment-body" }, [
              meta ? el("span", { class: "cert-meta" }, [meta]) : null,
              el("h3", null, [t(m.title)]),
              el("p", null, [t(m.text)]),
              m.personal ? el("p", { class: "moment-personal" }, [t(m.personal)]) : null,
              el("div", { class: "moment-links" }, links),
            ]),
          ])
        );
      });
    ...
    items.forEach((item) => {
      container.appendChild(
        el("div", { class: "highlight-item" }, [
          el("div", { class: "highlight-number" }, [String(fmt(item.number))]),
          el("div", { class: "highlight-label" }, [fmt(item.label)]),
        ])
      );
    });
  }
  ...
  function setupContactForm() {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        if (CONTACT_CONFIG.web3formsKey || CONTACT_CONFIG.formspreeEndpoint) {
          submitBtn.disabled = true;
          show(c.formSending, "info");
          const useW3F = !!CONTACT_CONFIG.web3formsKey;
          const res = await fetch(useW3F ? "https://api.web3forms.com/submit" : CONTACT_CONFIG.formspreeEndpoint, {
            method: "POST",
            headers: { "Content-Type": "application/json", Accept: "application/json" },
            body: JSON.stringify(
              useW3F
                ? { access_key: CONTACT_CONFIG.web3formsKey, from_name: data.name, name: data.name, email: data.email, subject: data.subject, message: data.message }
                : data
            ),
          });
          ...
          if (res.ok && json.success !== false) {
            form.reset();
            show(c.formSuccess, "ok");
          } else {
            show(c.formError, "error");
          }
        } else {
          // Sem serviço configurado: abre o programa de email com tudo preenchido
          const body = `${data.message}\n\n— ${data.name} (${data.email})`;
          const href = `mailto:${CONTACT_CONFIG.email}?subject=${encodeURIComponent(data.subject)}&body=${encodeURIComponent(body)}`;
          show(c.formMailto, "info");
